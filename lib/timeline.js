...
Timeline.prototype.onTimeUpdate = function(event){
	...
	*/
	if (this.current_ticker != this.timeToX(this.time)) {
		this.updateGUI();
	}
}
/**
	...
	if(pos.y > this.options.scaleHeight && pos.y < this.options.height){
		for(var i=0; i<this._periods.length; i++){
			...
			if(mouse_time > period.time_in && mouse_time < period.time_out && pos.y > this.yTrack(period.track).top && pos.y < this.yTrack(period.track).bottom){
				...
				else if (this.options.mode == 'read'){
					$(this).trigger('period_clicked', period)
				}
...
Timeline.prototype.onMouseUp = function(event){
	...
	if(this.options.mode == 'create'){
		...
		if(initial_point.x != final_point.x){
			$(this).trigger('period_draw', {'time_in': this.xToTime(initial_point.x), 
											'time_out': this.xToTime(final_point.x)})
		}
	}
}
/**
...
Timeline.prototype.onCanvasMouseMove = function(event){
	...
	// Find a possible period of mouseover
	else if (this._onperiod == null) {
		for(var i=0; i<this._periods.length; i++){
			var period = this._periods[i];
			if(mouse_time > period.time_in && mouse_time < period.time_out && pos.y > this.yTrack(period.track).top && pos.y < this.yTrack(period.track).bottom){
				this._onperiod = period;
				$(this).trigger('period_mouseover', {period: period, pos: pos})
				break;
			}
		}
	}
	// Check possible period of mouseout of period
	else {
		var period = this._onperiod;
		if(mouse_time > period.time_in && mouse_time < period.time_out && pos.y > this.yTrack(period.track).top && pos.y < this.yTrack(period.track).bottom){
		}
		else {
			this._onperiod = null;
			$(this).trigger('period_mouseout', {period: period, pos: pos})
		}
	}
...
Timeline.prototype.updateGUI = function() {
	...
	// time ticker (cursor)
	this.current_ticker = this.timeToX(this.time);
	this.drawLine(this.current_ticker, 0, this.current_ticker, this.options.cursorHeight, this.options.cursorColor);
...
Timeline.prototype.addPeriod = function(time_in, time_out, color, track, label, id){
	if(id == undefined){(((1+Math.random())*0x10000)|0).toString(16).substring(1)}
	...
	if (period.time_out - period.time_in < 1) {period.time_out += 1}
 ...
 * @private
 */
Timeline.prototype.deletePeriod = function(periodId){    
	for(var i=0; i<this._periods.length; i++){
		var period = this._periods[i];
...
Timeline.prototype.timeToX = function(time) {
	...
	if(this.scale){
    	return parseInt(time * this.scale.value * this.initialRatio);
	}
	else {
		return parseInt(time * 1 * this.initialRatio);
	}
}
/**
 * @private
